chrome.runtime.onMessage.addListener(function (message, sender, sendResponse) {
    ...
    if (message.from == "popup") {
        ...
        if (location.host == "forms.office.com") {
            const n = message.contents.id-1
            const elem = document.getElementById("question-list");
            const child = elem.childNodes[n];
            const opt = child.querySelector("input[data-automation-id='textInput']");
            if (opt) {
                opt.value = message.contents.content
            } else {
                const choice = child.querySelectorAll("div[data-automation-id='choiceItem']");
                choice.forEach(element => {
                    if (element.innerText == message.contents.content) {
